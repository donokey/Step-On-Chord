...
| 格式 | 实现路径 | 说明 |
|---|---|---|
| ChordPro | 纯 TS 文本生成 | 因歌词与和弦不对齐，v1 不做行内嵌和弦；2026-09-10 修订为标准兼容方言（B3 查验发现 `[段落名]` 与 ChordPro 行内和弦标记语法冲突，第三方解析器会拆成假和弦）：头部 {title}/{key}/{tempo} + 段落标题用 {comment: 段落名} + 纯和弦行 [C] [G] [Am] + 歌词附在文末 |
...
- `midi-writer-js`（MIDI 生成，纯 JS）——⚠️ 2026-08-11 用户决定 MIDI 暂缓导出，本依赖暂不引入
- `vitest`（前端纯逻辑单测，dev 依赖）——已引入
- `docx`（歌词导出 docx 生成，纯 JS）——✅ 2026-08-11 用户确认引入
...
1. **项目默认存放位置**：✅ 2026-08-11 决定——集中在 `<文档>/StepOnChord Projects`（settings.projectsRoot，可配），新建项目不再每次弹框选位置。
2. **音频默认模式**：reference（省空间，现状语义）还是 copy（防丢，占空间）？建议默认 reference + 醒目提供收集按钮。
3. **MIDI 内容**：✅ 2026-08-11 决定——MIDI 暂缓导出（用户：乐谱为主）；若日后恢复，再定音轨结构。
...
7. **PDF 模板样式 / ChordPro 方言**：✅ 2026-08-11 按 spec 3.4 既定方案落地（ChordPro：头部指令+按段落和弦+歌词附文末；PDF：简版浅色 A4 模板先行），用户确认走默认。2026-09-10 B3 手工查验（ChordSheetJS 实测）发现原方言的 `[段落名]` 段落头与标准行内和弦标记语法冲突，已改为标准兼容方言（见 3.4 修订记录）。
