...
[Step On Chord] 从 AI-ChordCraft src/chord_recognition.py 原样迁移（该文件无 LLM 依赖），
仅修改 _default_plkd_acr_dir()：默认模型目录指向 resources/models/acr_model。
"""
...
def _classify_single_chord_segment_with_numpy_wav(
    ...
    score_kwargs: dict[str, Any] = {"top_k": top_k}
    if qualities is not None:
        score_kwargs["qualities"] = qualities
    ...
    return {
        ...
        "chord_vocabulary": "extended" if qualities is not None else "basic_major_minor",
...
EXTENDED_CHORD_QUALITIES = tuple(CHORD_QUALITY_TEMPLATES.keys())


def refine_chord_event_qualities(
    audio_path: str,
    chord_events: list[dict[str, Any]],
    key: Any = None,
    max_segment_seconds: float = 12.0,
    min_segment_seconds: float = 0.6,
    upgrade_margin: float = 0.03,
    added_tone_ratio: float = 0.55,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """保守地把三和弦事件的 quality 升级为七/六/延伸和弦。

    BTC 引擎只输出 major/minor。这里对每个和弦区间切片做 16 模板 HPCP 匹配，
    保留 BTC 根音，仅当同根音的扩展 quality 模板得分超过同根音基础 quality
    模板得分 upgrade_margin 以上时才替换，避免过度预测七和弦。
    任何单区间失败都保留原事件，不中断主流程。
    """
    ...
    full_audio: Any = None
    try:
        loaded, loaded_sr = _sf.read(audio_path, dtype="float32", always_2d=False)
        if getattr(loaded, "ndim", 1) > 1:
            loaded = loaded.mean(axis=1)
        if int(loaded_sr) != slice_rate:
            import librosa as _librosa

            loaded = _librosa.resample(loaded, orig_sr=int(loaded_sr), target_sr=slice_rate)
        full_audio = loaded
    except Exception:
        full_audio = None
    ...
    refined = [dict(event) for event in chord_events if isinstance(event, dict)]

    with _tempfile.TemporaryDirectory(prefix="chordcraft-refine-") as temp_dir:
        for index, event in enumerate(refined):
            start = event.get("time_seconds")
            ...
            if duration < min_segment_seconds or duration > max_segment_seconds:
                ...
                continue
            chord = normalize_chord_symbol(event.get("chord"))
            root = _root_from_symbol(chord) if chord else None
            if not root:
                stats["skipped"] += 1
                continue

            stats["considered"] += 1
            segment_path = str(Path(temp_dir) / f"refine-{index:03d}.wav")
            try:
                if full_audio is not None:
                    s0 = int(start * slice_rate)
                    s1 = min(int(math.ceil(end * slice_rate)), len(full_audio))
                    if s0 >= len(full_audio) or (s1 - s0) < int(0.25 * slice_rate):
                        stats["skipped"] += 1
                        continue
                    _sf.write(segment_path, full_audio[s0:s1], slice_rate, subtype="PCM_16")
                else:
                    slice_audio(audio_path, segment_path, start, end)
                result = _classify_single_chord_segment_with_numpy_wav(
                    segment_path,
                    key=key,
                    top_k=len(CHORD_QUALITY_TEMPLATES) * 12,
                    qualities=EXTENDED_CHORD_QUALITIES,
                )
            except Exception:
                stats["errors"] += 1
                continue
            ...
            best_basic = max(
                (item for item in same_root if item.get("quality") in BASIC_CHORD_QUALITIES),
                key=lambda item: float(item.get("score") or 0.0),
                default=None,
            )
            best_ext = max(
                (item for item in same_root if item.get("quality") not in BASIC_CHORD_QUALITIES),
                key=lambda item: float(item.get("score") or 0.0),
                default=None,
            )
            if best_basic is None or best_ext is None:
                stats["skipped"] += 1
                continue
            margin = float(best_ext.get("score") or 0.0) - float(best_basic.get("score") or 0.0)
            if margin < upgrade_margin:
                continue

            # 声学证据门：以 BTC 原始三和弦为基准，扩展音（如 maj7 的大七度音）
            # 在 chroma 里必须真实可闻，否则视为泛音/混音染色，不升级。
            chroma = result.get("chroma") or []
            if isinstance(chroma, list) and len(chroma) == 12:
                root_pc = PITCH_CLASSES.get(str(root))
                body = str(chord)[1:] if chord else ""
                event_is_minor = body.startswith("m") and not body.startswith("maj")
                basic_intervals = [0, 3, 7] if event_is_minor else [0, 4, 7]
                ext_intervals = [
                    interval
                    for interval, _ in CHORD_QUALITY_TEMPLATES.get(best_ext.get("quality"), ("", []))[1]
                ]
                if root_pc is not None and ext_intervals:
                    chroma_index = (root_pc - 9) % 12
                    triad_energy = sum(
                        float(chroma[(chroma_index + t) % 12]) for t in basic_intervals
                    ) / len(basic_intervals)
                    added = [p for p in ext_intervals if p not in set(basic_intervals)]
                    if triad_energy > 0 and added:
                        weakest = min(float(chroma[(chroma_index + p) % 12]) for p in added)
                        if weakest < added_tone_ratio * triad_energy:
                            continue

            new_symbol = normalize_chord_symbol(best_ext.get("chord"))
            if not new_symbol or new_symbol == chord:
                continue
            event["chord"] = new_symbol
            event["display_chord"] = new_symbol
            event["quality_refined"] = True
            stats["upgraded"] += 1
...
def _default_plkd_acr_dir() -> Path:
    ...
    # Step On Chord 桌面版：模型统一放 resources/models/（engine_main.py 会设置 CHORDCRAFT_MODEL_DIR）
    model_dir = os.environ.get("CHORDCRAFT_MODEL_DIR")
    if model_dir:
        return Path(model_dir).expanduser().resolve() / "acr_model"
    return Path(__file__).resolve().parents[1] / "resources" / "models" / "acr_model"
