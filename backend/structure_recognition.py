"""SongFormer 曲式分割（仅保留 local 嵌入式模式）。

从 AI-ChordCraft 的 src/structure_recognition.py 精简：
- 删除 HTTP 远程服务分支（read_songformer_base_url / recognize_structure_with_songformer_service）
- 删除 requests 依赖与 mode 分发器，调用方直接使用 recognize_structure_with_songformer_local()
- 默认模型根目录改为 resources/models/SongFormer（可用 CHORDCRAFT_SONGFORMER_ROOT 覆盖）
"""
...
class StructureRecognitionError(RuntimeError):
    """Raised when the SongFormer structure-recognition engine fails."""
...
def _default_songformer_root() -> Path:
    ...
    if model_dir:
        return Path(model_dir).expanduser().resolve() / "SongFormer"
    return Path(__file__).resolve().parents[1] / "resources" / "models" / "SongFormer"
...
def _load_local_songformer_module() -> Any:
    ...
    songformer_root = Path(
        os.environ.get("CHORDCRAFT_SONGFORMER_ROOT") or str(_default_songformer_root())
    ).resolve()
...
def recognize_structure_with_songformer_local(
    ...
    *,
    ...
    """本地加载 SongFormer 模型做曲式分割，返回 (sections, raw_info)。"""
